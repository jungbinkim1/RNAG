...
t_ode, x_ode, f_ode = SIRNAG_C_optimizer(iss_ode * T + 1, x0, 1 / iss_ode, xi, cost, grad, exp, log, transp)
t1, x1, f1 = RNAG_C_optimizer(math.floor(iss1 * T) + 1, x0, (iss1 ** 2), xi, cost, grad, exp, log, transp)
t2, x2, f2 = RNAG_C_optimizer(iss2 * T + 1, x0, (iss2 ** 2), xi, cost, grad, exp, log, transp)
t3, x3, f3 = RNAG_C_optimizer(iss3 * T + 1, x0, (iss3 ** 2), xi, cost, grad, exp, log, transp)
...
plt.show()
filename='results/convex_graph_final.eps'
...
ax.legend(fontsize=15)
plt.show()
filename='results/convex_distance_final.eps'
fig.savefig(filename, format='eps')